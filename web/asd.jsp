...
<script>
    $(document).ready(function() {
        ...
        // 如果用户名不为空，则显示用户信息、设置和退出按钮；否则只显示登录按钮
        if (userName !== "") {
            // 显示用户信息、设置和退出按钮
            $("#userinfo").text(userName); // 更新导航栏中的用户名
            $(".dropdown-menu .dropdown-item:contains('退出')").show(); // 隐藏退出按钮
            $(".dropdown-menu .dropdown-item:contains('登录')").hide(); // 显示登录按钮
        } else {
            // 只显示登录按钮
            $("#userinfo").text("未登录"); // 更新导航栏中的文本为"登录"
